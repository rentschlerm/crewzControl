  ...
  useEffect(() => {
    const fetchResources = async () => {
      ...
      try {
        ...
        const url = `https://CrewzControl.com/dev/CCService/GetSkillList.php?DeviceID=${encodeURIComponent(
          deviceInfo.id
        )}&Date=${formattedDate}&Key=${key}&AC=${authorizationCode}&CrewzControlVersion=${crewzControlVersion}&Quote=${quoteSerial}&Longitude=${location.longitude}&Latitude=${location.latitude}`;
  
        console.log("Fetch Skills URL:", url);
        ...
        if (result.ResultInfo?.Result === "Success") {
          const skills =
            Array.isArray(result.ResultInfo.Selections?.Skill) 
              ? result.ResultInfo.Selections.Skill.map((skill: any) => ({
                  id: skill.Serial || Math.random(), // Use Serial or a fallback ID
                  name: skill.Name,
                }))
              : [
                  {
                    id: result.ResultInfo.Selections?.Skill?.Serial || Math.random(),
                    name: result.ResultInfo.Selections?.Skill?.Name || "Unknown",
                  },
                ];
          setResourcePackages(skills);
        } else {
          Alert.alert("Error", result.ResultInfo?.Message || "Failed to fetch skills.");
        }
      } catch (error) {
        console.error("Error fetching skills:", error);
        Alert.alert("Error", "An error occurred while fetching skills.");
      } finally {
        setLoading(false);
      }
    };

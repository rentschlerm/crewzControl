...
const AddEquipmentsGroup: React.FC = () => {
  ...
  useEffect(() => {
    const fetchResources = async () => {
      ...
      try {
        ...
        const url = `https://CrewzControl.com/dev/CCService/GetEquipmentList.php?DeviceID=${encodeURIComponent(
          deviceInfo.id
        )}&Date=${formattedDate}&Key=${key}&AC=${authorizationCode}&CrewzControlVersion=${crewzControlVersion}&Longitude=${location.longitude}&Latitude=${location.latitude}`;
          console.log('Equipment List URL: ', url);
        ...
        if (result.ResultInfo?.Result === "Success") {
          const equipments =
            Array.isArray(result.ResultInfo.Selections?.equipment)
              ? result.ResultInfo.Selections.equipment.map((equipment: any, index: number) => ({
                  id: equipment.Serial || `generated-${index}`, // Use `Serial` or fallback
                  name: equipment.Name,
                }))
              : [
                  {
                    id: result.ResultInfo.Selections?.equipment?.Serial || "generated-0",
                    name: result.ResultInfo.Selections?.equipment?.Name,
                  },
                ];
  
          setResourcePackages(equipments);
        } else {
          Alert.alert("Error", result.ResultInfo?.Message || "Failed to fetch equipment list.");
        }
      } catch (error) {
        console.error("Error fetching equipment list:", error);
        Alert.alert("Error", "An error occurred while fetching equipment list.");
      } finally {
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    style={[
                      styles.item,
                      selectedPackages.has(item.id) ? styles.itemSelected : styles.itemUnselected,
                    ]}
...
export default AddEquipmentsGroup;
